...
test('getPeopleNamesOlderThan() returns an array of people full names older than a certain age(older than 20), e.g. "Luke Skywalker"', () => {
...
test("getPeopleByLastName() returns an array of people of a certain lastName (Doe)", () => {
  const result = getPeopleByLastName(people);
...
test("findPersonById() returns the full object of a person", () => {
  const result = findPersonById(people);
  expect(result).toEqual({
    id: 5,
    firstName: "Luke",
    lastName: "Skywalker",
...
test("isAnyoneOlderThan() returns true if any person is older than the given age", () => {
  const result = isAnyoneOlderThan(people);
